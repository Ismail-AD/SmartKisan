...
import com.appdev.smartkisan.presentation.feature.Seller.inbox.ListRowData
                ...
                when (state.currentTab) {
                    0 -> {
                        when {
                            ...
                            else -> MessagesList(messages = state.recentMessages) { id, name, image ->
...
@Composable
fun MessagesList(messages: List<ChatMateData>, messageUser: (String, String, String?) -> Unit) {
    LazyColumn(
        ...
        itemsIndexed(
            ...
            Column {
                ListRowData(
                    ...
                    messageUser(
                        eachContact.partnerId!!,
                        eachContact.receiverName!!,
                        eachContact.receiverImage!!
                    )
                }
...
fun ChatWithList(chatWithList: List<UserEntity>, messageUser: (String, String, String?) -> Unit) {
    LazyColumn(
        ...
        itemsIndexed(
            items = chatWithList,
            key = { _, seller -> seller.id ?: "" }
        ) { index, seller ->
            SellerItem(seller = seller) {
                seller.id?.let { messageUser(it, seller.name, seller.imageUrl) }
            }

            if (index < chatWithList.lastIndex) {
                HorizontalDivider(
                    modifier = Modifier.padding(horizontal = 10.dp),
                    thickness = 1.5.dp,
                    color = Color.Gray.copy(alpha = 0.35f)
