...
class ChatBotRepository @Inject constructor(
    ...
    ): Flow<ResultState<com.appdev.smartkisan.domain.model.BotChatMessage>> = flow {
        try {
            ...
            val chatMessage = com.appdev.smartkisan.domain.model.BotChatMessage(
                id = messageId,
                message = message,
                role = role,
    ...
    suspend fun getTodayMessages(): Flow<ResultState<List<com.appdev.smartkisan.domain.model.BotChatMessage>>> = flow {
        try {
            emit(ResultState.Loading)
            ...
            val messages = firestore.collection(CHAT_COLLECTION)
                ...
                .get()
                .await()
                .toObjects(com.appdev.smartkisan.domain.model.BotChatMessage::class.java)
    ...
    suspend fun getMessagesByDate(date: String): Flow<ResultState<List<com.appdev.smartkisan.domain.model.BotChatMessage>>> = flow {
        try {
            emit(ResultState.Loading)
    ...
    suspend fun loadMoreMessages(
        date: String,
        lastTimestamp: Long
    ): Flow<ResultState<List<com.appdev.smartkisan.domain.model.BotChatMessage>>> = flow {
        try {
            emit(ResultState.Loading)
